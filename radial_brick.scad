...
Build one-stud-wide radial-circle bricks from a circumference stud count rather
		...
		union() {
			...
			if (archway > 0) {
				...
				_radial_brick_archway(
					...
					profile_padding=WALL_THICKNESS,overlap=0,frame=1
				);
			}
				...
				for (i = [0:pin_count-1])
					rotate([0, 0, start_angle+(i+.5)*angle_pitch])
						union() {
							...
							translate([inner_radius+1, -WALL_THICKNESS/4, STUD_HEIGHT+.02])
								cube([
...
module _radial_brick_archway(inner_radius, outer_radius, brick_height,
		...
		profile_padding=0,overlap=0,frame=.5) {
