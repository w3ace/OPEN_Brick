...
Build one-stud-wide grid-circle bricks from a circumference stud count rather
than a nominal radius. Adjacent stud centres are always one 8 mm stud pitch apart.
...
function grid_circle_brick_radius(circumference_studs, stud_pitch=BRICK_WIDTH) =
...
module grid_circle_brick(studs=4, circumference_studs=40, height=3,
		start_angle=0, studs_on_top=true, attributes=[]) {
	angle_pitch = 360/circumference_studs;
	center_radius = grid_circle_brick_radius(circumference_studs);
	...
	assert(circumference_studs >= 4 && circumference_studs == floor(circumference_studs),
		"grid_circle_brick: circumference_studs must be an integer of at least 4");
	assert(studs >= 1 && studs == floor(studs),
		"grid_circle_brick: studs must be a positive integer");
	assert(studs <= circumference_studs,
		"grid_circle_brick: studs cannot exceed circumference_studs");
	assert(height > 0, "grid_circle_brick: height must be positive");
	assert(archway <= 0 || studs >= 3,
		"grid_circle_brick: archway requires at least 3 studs");
	assert(archway <= 0 || height*PLATE_HEIGHT > WALL_THICKNESS,
		"grid_circle_brick: archway height must exceed the top-wall thickness");
...
module _grid_circle_brick_archway(inner_radius, outer_radius, brick_height,
...
module _grid_circle_brick_masonry(inner_radius, outer_radius, brick_height,
	...
	for (z = [-joint_width/2, brick_height-joint_width/2])
		_grid_circle_brick_masonry_faces(
...
module _grid_circle_brick_masonry_faces(inner_radius, outer_radius, z, height,
		angle_start, angle_sweep, depth, bevel_angular=true) {
	overlap = .08;
	bevel_angle = 45;
	_grid_circle_brick_beveled_joint(
...
module _grid_circle_brick_beveled_joint(surface_radius, bottom_radius, z, height,
...
module _grid_circle_brick_wedge(inner_radius, outer_radius, wedge_height,
