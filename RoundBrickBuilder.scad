	...
	difference() {
		intersection() {
			union() {
				...
				makeRoundStuds(outer_radius,inner_radius,height);
				makeRoundAntistuds(outer_radius,inner_radius,height);
			}
			rotate_extrude()
				translate([inner_radius*BRICK_WIDTH,0,0])
					square([(outer_radius-inner_radius)*BRICK_WIDTH,height*PLATE_HEIGHT*1.2]);
		}
		makeRoundStuds(outer_radius,inner_radius,0,studstyle=4);	
	}
}
